...
public abstract class SaslServerHandler<M> extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        try {
            ...
            if (!server.isComplete()) {
                ctx.writeAndFlush(newContinueMessage(Unpooled.wrappedBuffer(challenge)));
            } else {
                ctx.writeAndFlush(newSuccessMessage(Unpooled.wrappedBuffer(challenge)));
    ...
    /**
     * Creates a new message which signals the remote peer the success of negation.
     *
